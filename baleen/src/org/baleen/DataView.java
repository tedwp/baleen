...
import org.baleen.gui.RadarPoints;
...
public class DataView {
	...
	public DataView(MixContext ctx) {
		Log.d(MixView.TAG, "Debug: DataView entered");
		this.mixContext = ctx;
	}
	...
	public void doStart() {
		Log.d(MixView.TAG, "Debug: DataView - doStart entered");
	...
	public void init(int widthInit, int heightInit) {
		Log.d(MixView.TAG, "Debug: DataView - init entered");
		try {
			width = widthInit;
			height = heightInit;
...
//					request.url = TWITTER_HOME_URL +"?geocode="+lat + "%2C" + lon+"%2C" + radius + "km" ;
					request.url = TWITTER_HOME_URL +"?geocode="+lat + "," + lon+"," + radius + "km" ;
	...
	public void clearEvents() {
		Log.d(MixView.TAG,"Debug: DataView - clearEvents entered");
		synchronized (uiEvents) {
			uiEvents.clear();
		}
	}
